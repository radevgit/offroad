...
use crate::{
    arc::{arc, arc_is_collapsed_ends, arc_is_collapsed_radius, arcline, contains_order2d},
...
fn split_line_line(arc0: &Arc, arc1: &Arc) -> Vec<Arc> {
    let mut res = Vec::new();
...
fn split_arc_arc(arc0: &Arc, arc1: &Arc) -> Vec<Arc> {
    let mut res = Vec::new();
    ...
    match inter {
        ...
        ArcArcConfig::NonCocircularTwoPoints(poin0, poin1) => {
            let mut p0 = poin0;
            let mut p1 = poin1;
            if contains_order2d(arc0.a, p0, p1) < 0f64 {
                (p1, p0) = (p0, p1);
            }
            ...
            if contains_order2d(arc1.a, p0, p1) < 0f64 {
                (p1, p0) = (p0, p1);
            }
        ...
        ArcArcConfig::CocircularOnePoint(_) => {}
        ArcArcConfig::CocircularTwoPoints(_, _) => {}
        ArcArcConfig::CocircularOneArc(_) => todo!(),
        ArcArcConfig::CocircularTwoArcs(_, _) => todo!(),
        ArcArcConfig::CocircularOnePointOneArc0(point, _) => todo!(),
        ArcArcConfig::CocircularOnePointOneArc1(point, _) => todo!(),
    }
    res
}
