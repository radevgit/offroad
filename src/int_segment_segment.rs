...
#[cfg(test)]
mod test_intersect_segment_segment {
    ...
    fn test_parallel_overlaping2() {
        ...
        match intersect_segment_segment(s0, s1) {
            SegmentConfig::TwoPoints(p0, p1, p2, p3) => {
                ...
                assert!(p1.close_enough(point(0.0, 0.0), ulp));
                assert!(p2.close_enough(point(2.0, 2.0), ulp));
    ...
    fn test_parallel_touching() {
        ...
        match intersect_segment_segment(s0, s1) {
            SegmentConfig::OnePoint(p0, par0, par1) => {
                assert!(p0.close_enough(point(1.0, 0.0), 1.0));
                assert!(close_enough(par0, 0.5, 1.0));
                assert!(close_enough(par1, -1.5, 1.0));
            }
            _ => panic!("Unexpected SegmentConfig variant"),
        }
    }
